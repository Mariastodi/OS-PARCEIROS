...
export function poolFor(category: string): string[] {
  ...
  return [
    ...new Set(
      category === "Misturado" ? Object.values(packs).flat() : packs[category],
    ),
  ];
}
...
export function drawWord(category: string, used: string[]) {
  ...
  return {
    word,
    used: [
      ...(exhausted ? used.filter((word) => !pool.includes(word)) : used),
      word,
    ],
  };
}
