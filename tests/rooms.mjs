...
async function post(action, session = {}, extra = {}, expected = 200) {
  const response = await fetch(`${base}/api/rooms`, {
    ...
    body: JSON.stringify({ action, pin: session.pin, ...extra }),
  });
...
async function get(s, expected = 200) {
  const response = await fetch(`${base}/api/rooms?pin=${s.pin}`, {
...
const joined = await Promise.all(
  Array.from({ length: 5 }, () => post("join", { pin: host.pin })),
);
const sessions = [
  host,
  ...joined.map((p) => ({ pin: host.pin, token: p.token })),
];
...
assert.equal(
  new Set(views.filter((v) => v.role === "parceiro").map((v) => v.word)).size,
  1,
);
for (const v of views) {
  assert.deepEqual(v.spies, []);
  assert.ok(v.players.every((p) => !("token" in p)));
  if (v.role === "impostor") assert.equal(v.word, null);
}
await Promise.all(sessions.map((s) => post("ready", s)));
const playing = await get(host);
assert.equal(playing.phase, "playing");
assert.ok(playing.endsAt > Date.now());
...
assert.equal(finish.room.spies.length, 2);
assert.ok(finish.room.word);
...
assert.equal(afterLeave.phase, "lobby");
assert.equal(afterLeave.players.length, 5);
assert.notEqual(afterLeave.host, created.room.me);
const newHost = sessions.find(
  (s, i) => i > 0 && views[i].me === afterLeave.host,
);
const victim = sessions.find((s) => s !== newHost && s !== host);
const victimView = await get(victim);
await post("kick", newHost, { playerId: victimView.me });
await get(victim, 401);
for (const s of sessions.slice(1).filter((s) => s !== victim))
  await post("leave", s);
await post("join", { pin: host.pin }, {}, 404);
await post("join", { pin: "bad" }, {}, 400);
console.log(
  "PASS: concurrent joins, automatic names, auth, roles privacy, synchronized clock, rematch, host transfer, removal, validation.",
);
